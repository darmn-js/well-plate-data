...
describe('Get plate labels', () => {
  const plate100Wells = new WellPlateData({ nbRows: 10, nbColumns: 10 });
  const plate100Labels = plate100Wells.getPlateLabels();
  it('Get plate labels from 10 x 10 well plate', () => {
    expect(plate100Labels[0]).toStrictEqual({ label: '1-1' });
    expect(plate100Labels[1]).toStrictEqual({ label: '1-2' });
    expect(plate100Labels[98]).toStrictEqual({ label: '1-99' });
    expect(plate100Labels[99]).toStrictEqual({ label: '1-100' });
  });

  const plate96Wells = new WellPlateData();
  const plate96Labels = plate96Wells.getPlateLabels();
  it('Get plate labels from H x 12 well plate', () => {
    expect(plate96Labels[0]).toStrictEqual({ label: '1-A1' });
    expect(plate96Labels[1]).toStrictEqual({ label: '1-A2' });
    expect(plate96Labels[94]).toStrictEqual({ label: '1-H11' });
    expect(plate96Labels[95]).toStrictEqual({ label: '1-H12' });
  });
});
